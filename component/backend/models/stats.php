 ...
 *
 */
class CmsupdateModelStats extends F0FModel
{
	/**
	...
	public function checkAndFixCommonTables()
	{
		...
		$dbInstaller = new F0FDatabaseInstaller(array(
			'dbinstaller_directory' => JPATH_ADMINISTRATOR . '/components/com_cmsupdate/sql/common'
		));
	...
	public function collectStatistics($useIframe)
	{
		...
		if (!file_exists(JPATH_ROOT . '/administrator/components/com_cmsupdate/lib/stats/usagestats.php'))
		{
			return false;
		}
        ...
        // Since there isn't any version.php file, I have to query the extension table
        $query = $db->getQuery(true)
                    ->select($db->qn('manifest_cache'))
                    ->from($db->qn('#__extensions'))
                    ->where($db->qn('type').' = '.$db->q('component'))
                    ->where($db->qn('element').' = '.$db->q('com_cmsupdate'));
		...
		$at_parts = explode('.', $manifest->version);
		$at_major = $at_parts[0];
